from flaskr.model import Team, Game, SeasonResult
...
def filterByNTeams(games, opps):
    ...
    for game in games:
        for opp in opps:
            ...
            if((opp == game.home.name or opp == game.away.name) and (game.input.teamOFName == game.home.name or game.input.teamOFName == game.away.name)):
...
def calcNTeamSeasonResult(file_path, input):
    with open(file_path, 'r') as f:
        ...
        byTeamGames = filterByNTeams(gamesWithInputs, input.opps)
        numGames = 0
        for game in byTeamGames:
            numGames+=1
